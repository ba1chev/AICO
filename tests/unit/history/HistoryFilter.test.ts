...
const items: Calculation[] = [
  ...
  calc({ id: '3', hw: a100, rg: fr, hours: 8, kWh: 3.2, co2: 160, water: 1.6, date: new Date('2026-04-22'), label: 'Reserve' }),
];

describe('applyFilter', () => {
  it('filters by hardwareId', () => {
  ...
  it('search covers label, hardware and region', () => {
...
describe('applySort', () => {
  it('sorts by co2eGrams descending', () => {
    expect(applySort(items, 'co2eGrams', 'desc').map((c) => c.id)).toEqual(['1', '2', '3']);
  });

  it('sorts by energyKWh ascending', () => {
  ...
  it('sorts by hardware with locale-aware comparison', () => {
